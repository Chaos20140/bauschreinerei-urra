...
import { ArrowLeft, Download, Search, Loader2, RefreshCw, Paperclip } from 'lucide-react';
import { useAdmin, adminErrorText, type JobRow } from '../../lib/admin';
import { STATUS_META, STATUS_ORDER, formatDate, type StatusKey } from './contactFields';
...
function haystack(r: JobRow): string {
  return [r.name, r.email, r.phone, r.position, r.message]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
}
...
export function AdminJobs() {
  const { listJobs, exportJobs } = useAdmin();
  ...
  const load = useCallback(async () => {
    ...
    setErr('');
    try {
      setRows(await listJobs(includeArchived));
    } catch (e) {
      setErr(adminErrorText(e));
    } finally {
      setLoading(false);
    }
  }, [listJobs, includeArchived]);
  ...
  const onChanged = (updated: JobRow) =>
  ...
  async function doExport() {
    ...
    setExporting(true);
    try {
      await exportJobs(includeArchived);
    } catch (e) {
      setErr(adminErrorText(e));
    } finally {
      setExporting(false);
    ...
    <section className="max-w-5xl mx-auto px-6 py-10">
      <div className="flex items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <h1 className="hero-title text-white text-2xl md:text-3xl font-medium">Bewerbungen</h1>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <p className="text-white/50 text-sm py-16 text-center">
          {rows.length === 0 ? 'Noch keine Bewerbungen eingegangen.' : 'Keine Treffer für diese Filter.'}
        </p>
      ) : (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      {r.has_cv && (
                        <Paperclip size={13} className="text-white/40 shrink-0" aria-label="mit Lebenslauf" />
                      )}
                      {r.admin_archived && (
                    ...
                    <span className={`shrink-0 text-[11px] px-2.5 py-1 rounded-full border ${meta.badge}`}>
                      {meta.label}
                    </span>
                  </div>
                  <div className="mt-2 flex items-center justify-between gap-3 text-sm">
                    <span className="text-white/60 truncate">
                      {r.position || 'Initiativbewerbung'}
                    </span>
                    <span className="text-white/40 text-xs shrink-0">{formatDate(r.created_at)}</span>
                  </div>
                </button>
              </li>
            );
